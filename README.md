...
此外，**引擎 / 语言 / 输出模式 / 精细度 / VAD / 批量 / 术语表 / 近音词纠正** 等界面选项也会自动记住，
下次启动恢复上次选择（关闭窗口时保存）。
...
### 近音词纠正：提示词兜不住的部分用拼音确定性补上

提示词是**概率手段**——实测同一段音频里「德国管理应用技术大学」被修好了，
而「生源质量」仍会错成「声源质量」，换个引擎/换个时间结果还可能不同。
故在术语表之外再加一层**确定性**后处理（`app/homophone.py`，界面「按术语表纠正近音词」，默认开）：
把术语表的拼音与转写结果逐字比对，**拼音完全一致但写法不同**才替换。
...
在历史真实转写稿上 5/5 全部修回；在人工修正稿（正确文本）上改动数 **0**，即无误修。

为什么用拼音而不是字形：拼音一致才替换，**「成都工业大学」与「成都工业学院」拼音不同 → 不会被误改**。
替换等长，故只改字、不动时间轴。

安全阈值（`MIN_SUBSTR`）来自实测：用长术语的 2 字子串参与匹配会撞上常见词——
「德国管理应用技术大学」的子串「的国」会命中「自**己的国**际化」（的/德 同为 de）：
...
故按术语长度分层：术语 ≥3 字时其 ≥3 字子串参与匹配（容忍术语内部还有别的错），
≤2 字时只认整词（不做子串扩张）。

**修不了的（明确不做，避免硬猜误伤）**：近音混淆（先生/新生）、插入删除（迎接来/迎来）、
选词错误（系统引入/系统引进）——这三类需要对齐与语言模型能力，确定性替换无能为力。
...
| 中断下载不冒充完整 | 下载先写 `<文件名>.part`，**全部字节校验通过后才原子改名**为正式文件。否则取消/断网/崩溃/磁盘满会留下一个文件名正常但内容残缺的 `model.bin`，被「已下载」判定采信（实测报错为无从下手的 `Cannot load the target vocabulary from the model directory`）；`.part` 同时充当续传数据源 |
| mlx 时长准确 | Metal 引擎改用 PyAV 探测媒体真实时长，替代原先"末段结束时间"的近似值 |
| 解码器内存 | 内置 PyAV 解码改为**按容器时长预分配缓冲、逐帧原地写入**（不再累积帧对象再 `concatenate`）。30 分钟素材的解码额外开销 **181 MB → 0**（峰值 = 波形本体的 1.05x） |
| 分块特征提取 | faster-whisper 的 `FeatureExtractor` 对整段音频一次性做 STFT，中间态极大：30 分钟素材**多占 1.9 GB**（波形本身才 112 MB）。改为分块（每块 20 秒）执行，数学等价、**逐位一致**，额外峰值降至约 0；这是全链路最大的内存热点 |
| 立体声下混增益修正 | 重采样输出改用 `s16` 而非 `flt`：FFmpeg 对整型下混用 `(L+R)/2`、对浮点用 `(L+R)/√2`，后者高 **3 dB**。实测三个采访素材（峰值 0.87~0.89）被推到 1.23~1.26，其中 **0.83% 样本削波**到 ±1.0（实打实的失真）。改用 `s16` 后与 faster-whisper 的 `decode_audio` **逐位一致** |
| CPU 路径复用内置解码 | CPU/CUDA 路径此前完全交给 faster-whisper 自行解码（其内部用 `io.BytesIO` 累积 s16 再转 float32，峰值约 4.5x）。现改为传入预解码波形，并把解码放在模型加载**之前**（波形与权重不叠加驻留） |
| 内存总账 | 30 分钟音频端到端峰值 **2.2~2.7 GB → 1.63 GB**（转写输出逐段完全一致）；波形的 16 kHz × 4 B ≈ 230 MB/小时是不可压缩下限，优化只消除其上的叠加开销 |
...
| 批量导出不覆盖 | 输出文件名对**同名不同扩展名**（如 `采访.mp4` / `采访.wav`）补上源扩展名区分。此前会全部写入 `采访_转写稿.txt`，实测 3 个源文件最终只剩 1 份转写稿（静默丢 2 份）；不重名的文件仍保持简洁命名 |
| 导出命名跟随结果 | 批量转写后导出默认文件名依据**刚完成的结果**而非先前拖入的素材，避免文件名与内容来自不同素材 |
